...
import com.moufans.update.update.PermissionCheckerUtils;
import com.moufans.update.update.UpdateVersionDialog;
import com.trinidad.beansanconstruction.R;
import com.trinidad.beansanconstruction.ext.AppExtKt;
...
public class AppVersionUpdateUtils {
    ...
    public static void checkVersion(Activity context, OnAppUpdateListener onAppUpdateListener) {
        ...
        BaseExtKt.convertExecute(AppExtKt.getAppApi().checkVersion(version), new OnRequestListener<List<VersionDataBean>>(null) {

            @Override
            public void onSuccess(List<VersionDataBean> list) {
                if (list.size() <= 0) return;
                VersionDataBean bean = list.get(0);
                // 0:不需要更新 1：强制更新 2：更新
                // 是否强制更新（0/否，1/是）
